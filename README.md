...
- Database학습
    - 관계 데이터 모델
        - 무결성 제약조건
            - 슈퍼키 - 튜플을 유일한 값으로 구분 지울 수 있는 속성 또는 속성집합(고객번호 | 주민번호 |고객번호/이름 | 고객번호/주소 | 고객번호/이름/전화번호....| 이름만은 안됨)
            - 후보키 - 튜플을 유일한 값으로 구분 지울 수 있는 최소한의 속성집합(고객번호 | 주민번호)
            - 복합키 - 후보키 중 속성을 2개이상 집합으로 한 키
            - 기본키(Prinary Key!!) -  여러 후보키 중에서 하나를 선정하여 대표로 삼는 키 (고객번호)
                - 고려상항 - 고유한 값(Unique), NULL불가(Not Null), 최소 속성의 집합, 개인정보들의 보안사항은 자제
            - 대리키 - 기본키가 여러개의 속성으로 구성되어 복잡하거나, 보안문제가 생길때 새롭게 생성하는 키
            - 대체키 - 기본키로 선정되지 않은 후보키
            - 외래키(Foriegn Key!) - 기본키를 참조하여 사용하는 것
                - 고려사항 - 다른 릴레이션의 관계, 다른 릴레이션의 기본키를 호칭, 서로 같은 값이 사용, 기본키가 변경되면 외래키도 변경되어야 함,
                             NULL과 중복을 허용(Not NULL인 경우도 있음),자기자신의 기본키를 외래키로 사용할 수고 있음, 외래키가 기본키의 속성중
                             하나가 될 수도 있음

            - 데이터 무결성(Integrity) - DB에 저장된 데이터의 일관선과 정확성을 지키는 것
    ...
    - SELECT문
        - OUTER JOIN(외부조인)
            - LEFT|RIGHT|FULL(FULL은 거의 사용안함)
                ...
                - RIGHT OUTER JOIN(오른쪽 테이블을 기준으로 조건에 일치하지 않는 오른쪽테이블 데이터도 모두 표시)
...
- 파이썬 SQL Server 연동 프로그래밍
    - Madang DB 관리 프로그램
        ...
        - DB연결 설정 - Oracle, MySQL 등은 설정이 없음. 구성관리자에서 TCP/IP로 접근을 허용하지 않으면 접속 안됨
            ...
            ![구성관리자](https://github.com/sungouk1457/basic-database-2024/blob/main/images/db005.png)
...
- 파이썬 SQL Server 연동 프로그래밍
    ...
    - 실행화면
      ...
      https://github.com/sungouk1457/basic-database-2024/assets/131221230/2d3cda17-6e90-4085-8c89-e0eaa705da21

## 7일차
- SQL 고급
    - 트랜잭션 - All or Nothing
    - 트랜잭션 속성 - A(tomicity)-원자성, C(onsistency)-일관성, I(solation)-고립성, D(urability)-지속성
    - TCL에서 사용할 키워드
        - BEGIN, TRAN[SATION], COMMIT, ROLLBACK, SAVE
...
- 데이터베이스 모델링
    ...
    - 개념 모델링 - 요구상항을 받으면서 정해지지않은 여러 개체들을 정립화 하는 단계
...
- 인덱스 예제
    ...
    <!-- md주석.![인덱스](https://github.com/sungouk1457/basic-database-2024/blob/main/images/db006.png)-->
    <!-- html img태그가 이미지 사이즈 조정 가능-->
    <img src = "https://github.com/sungouk1457/basic-database-2024/blob/main/images/db006.png" width ="870">

## 8일차 
- 인덱스 예제
- 정규화
    - DB 상에서 생기는 이상현상(삽입,숙제,수정)이 생기지 않는 데이터베이스 설계
...
- 실무실습(사용자, 권한...)
    ...
    - hr DB를 사용할 사용자 계정을 생성, 필요한 권한 설정
        ...
        - SSMS > 보안 > 로그인 > sa존재 > 속성에서 비밀변경 이후 SQL Server 인증으로 sa 비밀번호 입력후 로그인 하면됨
        - SSMS > 보안 > 로그인 > 새 로그인 클릭
            - 사용자 계정 : hr_user, 비밀번호 : hr_p@ss!
            - 일반 : 기본 데이터베이스 hr 선택
            - 사용자매핑 : hr 선택, 데이터베이스 역할 멤버 : dn_owner 추가 선택
...
- 실무실습
    - 쿼리실습
     ...
     ![HR_ERD](https://github.com/sungouk1457/basic-database-2024/blob/main/images/db007.png)
